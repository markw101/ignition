...
$indexPage->render([
    'tableHeader' => [
        ...
        ['type' => 'textLabel', 'headerCell' => $model->fieldLabels['page_name']],
		['type' => 'textLabel', 'headerCell' => $model->fieldLabels['page_slug']],
    ...
    'tableRows' => function() use ($elements) {
		...
		foreach($elements as $data)
		{
		    ...
		    yield [
		        ['url' => BaseURL('/page' . $GLOBALS['KEYCODE'] . '/edit/' . $data['id'])],
		        $data['id'],
		        $data['page_name'],
		        $data['page_slug'],
		        $data['updated_at'],
		        $data['active'],
		        ['url' => BaseURL('/page' . $GLOBALS['KEYCODE'] . '/delete/' . $data['id'])]
		    ];
		}
	}
]);
