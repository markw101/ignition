...
<div class="panel-body clearfix" style="border:2px solid #008413; padding: 10px;">
    ...
    <center>
        ...
        <i class="fa fa-plus"></i> <?= lang('base.add_files') ?></button>
        <br>
...
<script>
    ...
    var fileDropzone = new Dropzone(document.body, { // Make the whole body a dropzone
        <?php // this one line causes the uploaded files to display on page load 
              // This is because when call upload without $_FILES, jumps to get_files ?>
        url: "<?= BaseURL('/asset' . $GLOBALS['KEYCODE'] . '/getfiles/'. $baseDirectory . '/')?>",
        params: {
        },
        thumbnailWidth: 80,
        thumbnailHeight: 80,
        ...
        init: function () {
            thisDropzone = this;
            $.getJSON("<?= BaseURL('/asset' . $GLOBALS['KEYCODE'] . '/getfiles/'. $baseDirectory . '/')?>",
                function (data) {
    ...
    fileDropzone.on('addedfile', function (file) {
        ...
        createDownloadButton(file, '<?= BaseURL("/asset" . $GLOBALS["KEYCODE"] . "/getfile/" . $baseDirectory) ?>' + '/' + '<?= $baseDirectory ?>' + '-' + file.name);
    });
