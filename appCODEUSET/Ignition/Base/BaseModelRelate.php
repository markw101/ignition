...
class BaseModelRelate extends BaseModel
{
    ...
    public $totalChildren;                          // if elect to save records, also save record count, may not equal total children records in database
    public $totChildRecordsDB;                      // total child records in databse vs total retrieved for operation
	...
	//       $this->parentIDValue
	//       returns returnList[]
	public function GetChildrenComplete($keyField2List, $idVal = FALSE, $processFromList = FALSE, $saveRawRecords = FALSE, $reverseOrder = FALSE, $maxReturn = FALSE, $pageNum = 1, $isCollection = FALSE) {
        ...
        if ($maxReturn && ($this->dbType == 'TRN' || $isCollection)) {
            ...
            $this->totChildRecordsDB = $recCount[0]['COUNT(*)'];

            // ----- create filter based on max return value IF table record count is greater than maximum return quantity
            if ($this->totChildRecordsDB > $maxReturn) {
                // ----- check for reverse order
                if ($reverseOrder)
                    $filter = ['__id_range__' => TRUE, 'offset' => $this->totChildRecordsDB - ($maxReturn * $pageNum), 'limit' => $maxReturn];
                else
	...
	public function SaveChildrenData($TableName = "__NOTSET__", $AllowedFields = "__NOTSET__") {
        ...
        // ----- built exception for taz store page views for bloglangs
        if ($TableName == 'bloglangs') {
		...
		foreach ($this->dataChildren as $child) {
			...
			if (strpos($childID, "TEMP") === 0) {
                ...
                // ----- insert a corresponding record in taz db to maintain page counts
                if ($TableName == 'bloglangs')
