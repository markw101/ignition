...
<blockquote>Simple vs Easy<br>What is simple is usually easy, what is easy is often not simple.  Simplicity and ease should not be confused.  Simplicity should be the target which shall increase functional efficiency, but shall not bolster ease to the detriment of simplicity.  Excessive complexity limits design potentials.</blockquote>
<h2>Features List</h2>
...
5) You must set the name of your website domain name in web/appCODEUSET/Site/SiteConstants.php using the SERVERBASENAME variable.  For example myignition.com.  This must match the name given for your web server.  Simply edit the file SiteConstants.php and input your domain name where it says SERVERBASENAME.
...
7) Locate the file named env in the web directory. This is the file used by CodeIgniter to apply your database name/username/password and other system settings. Copy this file to a file named .env. This file must begin with a dot in order for it to be located by CodeIgniter during execution. Edit this new file .env and make any changes required to match the database you set if you did not use the default database and password. If you went with the default database name and password simply copy the file to .env as no other changes should be required. If you are using a different database name then the default, then you will also have to edit several files and change them to reflect the new names given for the primary and secondary databases, ignitionbase and ignition_taz, respectively. To change the database names from the default, edit the file /web/appCODEUSET/Config/Database.php and input the new default database name that you chose. You will also need to change the values for the DEFAULTDBGROUP and DBGROUP variables in web/appCODEUSET/Site/SiteConstants.php to reflect the name of the new primary database you selected. Additionally, you must change the database name in the /web/appCODEUSET/Config/Session.php from ignition_taz to what ever you named the secondary database to in .env and in the actual database. For example you might rename your databases from ignitionbase and ignition_taz to mycompany and mycompany_taz respectively. Finally, the variables IGDBPREFIX and DBPREFIX must match the prefix given to the taz/secondary database.
8) Open your web browser and point it to <b>http://myignition.com</b>.  See Trouble shooting if you receive an error or this does not work.  You will also likely need to set this site name in your local hosts file in order for your operating system to identify the domain name myignition.com as being a localhost entry. In order to login as super admin go to the login url: <b>http://myignition.com/login</b> and enter the username of <b>admin@myignition.com</b> and leave the password empty or set to any value. WARNING: You must please never deploy this site to a public server without setting a password first which you may do in the user profile screen.  Additionally, you may set the login url ("login" by default) to a custom or obscure and hard to guess value such as loginX5T3A2Z1, in SiteConstants.php by setting the variable named LOGINURL.
...
<b>A reduction in directory count and the number of project files required:</b>

One thing that you may notice about Ignition is that it reduces the number of files and directories that are required to build a typical MVC module system. Typically, with the MVC, Model View Control, numerous directories are required and files to populate these directories to make a functioning MVC system. If one develops just a simple MVC module with a Model, View and Controller subdirectories, and each directory containing two or more files, then to make a very simple MVC module a total of 8-10 files/directories may be required. With a typical, small application requiring five such MVC systems, the total number of required files/subdirectories can easily climb to over 50! This can become quite cumbersome as much of the developer’s time may be consumed just with the file management tasks.

However, with the file arrangement system in Ignition the MVC module design maintains greater flexibility and, thereby, reduces the number of files and directories required. Often times the model and the controller are placed into one directory named MC and the remainder of the files are placed into a Views directory. A good example of this simplified design is in the appCODEUSET/Ignition/Taz module. In this module the two subdirectories are MC and Views. Inside of MC is the Taz controller which extends the BaseController and a single file in the Views subdirectory named TazLoginRequests.autoform.php. This also gives an example of the Ignition RAD - Rapid Application Development - autoform system, which further reduces the logistical complexity of the MVC system. See <em>Get RAD with Autoform</em> below for an explanation of this RAD build tool. Another example is the Login module (appCODEUSET/Ignition/Login). In this example the files required are placed in just one subdirectory, Views, and the Login directory itself. Study these two MVC module examples to learn more about this more flexible and simplified approach to MVC, and how to reduce project complexity. Start saving time while reducing file/directory counts in your next MVC project with Ignition.
...
<em>ONLY if you are running the wrong version of PHP you may switch between versions using the following instructions to change your distribution to run PHP 8.3, for example (you may replace the 8.3 with any version between 8.1 and above):</em>
